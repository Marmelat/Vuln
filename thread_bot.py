...
class IntelThread:
    def __init__(self):
        ...
        # YENİ: Tarayıcı Kimlik Havuzu (Rotation)
        self.user_agents = [
    ...
    async def send_telegram_card(self, message, link=None, search_query=None, extra_ref=None):
        ...
        try: async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as s: await s.post(url, json=payload, headers=self.headers)
        except: pass
    ...
    async def send_telegram_file(self, filepath):
        ...
        try: async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as s: await s.post(url, data=data)
        except: pass
    ...
    def extract_score(self, item):
        ...
        # HTML Title içinde "High" veya "Critical" varsa puanla
        if "critical" in txt: return 9.5
        if "high" in txt: return 8.0
        return 0.0
    ...
    async def enrich_score_from_web(self, url, current_score):
        """Web sayfasından gerçek CVSS v3 skorunu çeker"""
        if current_score > 0.0: return current_score # Zaten varsa dön
            ...
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
                async with session.get(url, headers=headers, timeout=20) as response:
                    if response.status == 200:
                        ...
                        # 1. Tenable Özel Formatı: "CVSS v3 ... Base Score: 7.1"
                        if "tenable.com" in url:
                            match = re.search(r"CVSS v3.*?Base Score:\s*([0-9\.]+)", text, re.DOTALL)
                            if match: return float(match.group(1))

                        # 2. Genel Format
                        match = re.search(r"(?:CVSS|Base Score|Score).*?(\d{1,2}\.\d)", text, re.IGNORECASE)
                        if match: return float(match.group(1))
                        
                        # 3. Kelime Bazlı Tahmin
                        if "Critical" in text: return 9.5
                        if "High" in text: return 8.0 
            return 0.0
        except: return 0.0
    ...
    async def format_alert_technical(self, item, header_title="ACİL UYARI"):
        ...
        # Ek skor bilgisi (Varsa v2 de gösterilebilir ama sade olsun diye v3 yetiyor)
    ...
    async def parse_generic(self, session, source, mode):
        # HATA KORUMALI & JITTER
        try:
            await asyncio.sleep(random.uniform(15.0, 45.0)) # Jitter
            ...
            if mode == "html_tenable":
                ...
                ua = random.choice(self.user_agents)
                headers = {"User-Agent": ua}
                async with session.get(source["url"], timeout=timeout, headers=headers) as r:
                    if r.status != 200: 
                    ...
                    for row in rows:
                        ...
                        if len(cols) >= 2:
                            ...
                            if link_tag:
                                ...
                                raw_id = link.split('/')[-1]
                                # Tablo satırında Critical/High yazıyorsa ön puan ver
                                score = 0.0
                                if "Critical" in str(row): score = 9.0
                                elif "High" in str(row): score = 7.5
                                items.append({"raw_id": raw_id, "title": title, "desc": "Tenable Plugin Update", "link": link, "score": score}) 
            ...
            final = []
            for i in items:
                # 1. Ham skoru al (Tablodan geldiyse)
                i['score'] = self.extract_score(i)
                # 2. Skor 0 ise ve Tenable/Teknik ise -> Deep Scout
    ...
    async def process_intelligence(self):
        ...
        logger.info("🔎 Tarama Sürüyor (v21.0 Deep Tenable)...")
        self.check_daily_reset()
        ...
        for threat in all_threats:
            ...
            if prev is None:
                ...
                if is_news: 
                    ...
                    # FİLTRE: 7.0+ (Tenable için 7.0 yeterli) veya Özel
                    if curr >= 7.0: notify = True
